...
We estimated the *population* parameter on time spent on email weekly.
...
boot_email
hours = boot_email%/%60
minutes = round(boot_email%%60)

paste(hours, "hours", minutes, "minutes")
...
approval_polls <- approval_polllist %>% 
  ...
  summarise(average_net = mean(net_approval_rate))


# Confidence Interval (CI) using the formula mean +- t_critical * SE
polls_formula_ci <- approval_polls %>% 
  mutate(avg_rate = mean(net_approval_rate, na.rm=TRUE)) %>%
  summarise(avg_rate = mean(net_approval_rate, na.rm=TRUE),
            ...
            rate_low = net_approval_rate - margin_of_error,
            rate_high = net_approval_rate + margin_of_error) +
            NULL


plot <- approval_polls %>% 
  ggplot(aes(x=enddate_week, y=average_net)) +
  geom_point()+
  geom_line()+
  geom_smooth(color="blue") +
  geom_ribbon(ymin=rate_low, ymax=rate_high) +
    #change to theme_bw() to have white background + black frame around plot
  theme_bw() +
  labs (title = "Net Biden Approval/Dissaproval Weekly",
    y = "Net Approval Rate") +
    geom_hline(yintercept = 0, color="orange") +
    NULL


#plot <- plot+geom_ribbon(aes(ymin=data$lower, ymax=data$upper), linetype=2, alpha=0.1)
plot 
...
#need to discuss values on X & Y axes
#need to discuss what to facet_wrap by
#need to understand how to interpret this graph
...
ggplot(hiv_life_expectancy_worldbank_data, aes(x = NY.GDP.PCAP.KD , y = SP.DYN.TFRT.IN)) +
  geom_point() +
  geom_smooth(method = "lm") +
   labs(
    title = "Relationship",
...
hiv_life_expectancy_worldbank_data_lessthan5 <- hiv_life_expectancy_worldbank_data %>% 
  filter(SH.DYN.MORT <=5) 

ggplot(hiv_life_expectancy_worldbank_data_lessthan5, aes(x = interval , y = SH.DYN.MORT)) +
  geom_point() +
  geom_smooth(method = "lm") +
   labs(
    title = "Relationship",
...
ggplot(hiv_life_expectancy_worldbank_data, aes(x = SP.DYN.TFRT.IN , y = SE.PRM.NENR)) +
  geom_point() +
  geom_smooth(method = "lm") +
   labs(
    title = "Relationship",
...
plot_bike <- bike %>% 
  ...
  mutate(fill = bikes_hired>expected_hired,
         up = ifelse(bikes_hired>expected_hired, bikes_hired-expected_hired, 0),
         down = ifelse(bikes_hired>expected_hired, 0,bikes_hired-expected_hired),
         Month = month(month, label=T))

plot_bike$down  = plot_bike$expected_hired - apply(plot_bike[,3:4],1,min) # creates a column taking the smallest value from actual vs average bikes hired
plot_bike$up = apply(plot_bike[,3:4],1,max) - plot_bike$expected_hired # creates a column taking the largest value from actual vs average bikes hired
plot_bike$date = ym(paste(plot_bike$year,plot_bike$month))

plot_bike %>%
  ggplot(aes(x=month)) +
  geom_ribbon(aes(ymin = bikes_hired, ymax = expected_hired+up),fill="#7DCD85",alpha=0.4)+
  geom_ribbon(aes(ymin = bikes_hired, ymax = expected_hired-down), fill="#CB454A", alpha=0.4)+
  geom_line(aes(y=expected_hired),colour="blue",size=2)+
  geom_line(aes(y=bikes_hired),colour="black",size=0.5)+
  facet_wrap(~year)+
  labs(title="Monthly changes in Tfl bike rentals",
       subtitle = "Change from monthly average shown in blue\n and calculated between 2016-2021",
       x="",
       y="Bike rentals",
...
# plot_bike %>% 
#   ggplot() +
#   geom_area(aes(x=Month,y=bikes_hired,group=year,fill="red")) + #fills the area of actual bikes hired in red (using scatter points as guide)
#   geom_area(aes(x=Month,y=expected_hired,group=year,fill="green"))+ # #fills the area of avg bikes hired in green (using scatter points as guide)
#   geom_area(aes(x=Month,y=lower,group=year,fill=NA))+
#   # geom_ribbon(aes(x=2,ymin=bikes_hired, ymax=expected_hired,fill="red"),alpha=.3)+
#   facet_wrap(~year)+
#   NULL

plot_bike %>%
  ggplot(aes(x=Month)) +
  geom_line(aes( y=expected_hired, group=year),colour="blue",size=2)+
  geom_line(aes(y=bikes_hired, group=year),colour="black",size=.5)+
  geom_ribbon(aes(ymin=expected_hired,ymax=expected_hired+up, group=year),fill="#7DCD85",alpha=0.4)  +
  geom_ribbon(aes(ymin=expected_hired,ymax=expected_hired+down, group=year),fill="#CB454A",alpha=0.4)+
  facet_wrap(~year)+ 
  theme_bw() + 
  labs(title = "Monthly changes in TfL bike rentals", 
...
expected_hires_week <- bike %>% 
  ...
  group_by(week) %>%
  summarise(expected_hired = mean(bikes_hired)) #outputs mean bike rentals by week with 52 rows
...
plot_bike_week <- bike %>% 
  ...
  inner_join(expected_hires_week, by = "week") %>% 
  mutate(fillcolor = bikes_hired>expected_hired,
         excess_rentals = bikes_hired - expected_hired,
         percentage_change_expected = (excess_rentals/expected_hired),
         up = ifelse(percentage_change_expected>0, (excess_rentals/expected_hired), 0),
         down = ifelse(percentage_change_expected>0, 0,bikes_hired-expected_hired))
...
plot_bike_week %>% ggplot() +
  ...
  geom_line(aes(x=week, y=percentage_change_expected)) + 
  geom_area(aes(x=week,y=percentage_change_expected)) +
  
  geom_rug(aes(x=week), color=ifelse(plot_bike_week$fillcolor,"green","red")) +
  facet_wrap(~year) +
