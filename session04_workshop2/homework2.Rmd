...
conf_high
conf_low
#the true mean of insta or snap users falls between 0.349 and 0.401
# gss %>% 
...
se_prop_twitter = sqrt(
  0.767 * 0.233 / 389
)

conf_low_usetwitter <-  0.233 - 1.96 * se_prop_twitter
conf_high_usetwitter <- 0.233 + 1.96 * se_prop_twitter

conf_low_notwitter <-  0.767 - 1.96 * se_prop_twitter
conf_high_notwitter <- 0.767 + 1.96 * se_prop_twitter
...
paste(boot_email%/%60, "hour", round(boot_email%%60), "minute")
...
expected_hires <- bike %>% 
  ...
  group_by(month) %>%
  summarise(expected_hired = mean(bikes_hired))

plot_bike <- bike %>% 
  filter(day>="2016-01-01")%>%
  group_by(year, month) %>%
  summarize(bikes_hired = mean(bikes_hired)) %>%
  inner_join(expected_hires, by = "month") %>%
  mutate(fill = bikes_hired>expected_hired) 
plot_bike$lower = apply(plot_bike[,3:4],1,min)
plot_bike$higher = apply(plot_bike[,3:4],1,max)
plot_bike$date = ym(paste(plot_bike$year,plot_bike$month))

plot_bike %>% 
  ggplot() +
  geom_area(aes(x=month,y=bikes_hired,group=year,fill="red"))+
  geom_area(aes(x=month,y=expected_hired,group=year,fill="green"))+
  geom_area(aes(x=month,y=lower,group=year,fill=NA))+
  #geom_ribbon(aes(x=2,ymin=bikes_hired, ymax=expected_hired,fill="red"),alpha=0.3)+
  facet_wrap(~year)+
  NULL
...
plot_bike %>%
  ...
  geom_line(aes(y=bikes_hired),colour="black",size=1)+
  geom_ribbon(aes(ymin = expected_hired, ymax = bikes_hired))+
  facet_wrap(~year)+
  NULL
