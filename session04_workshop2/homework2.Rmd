...
gss<-gss %>% mutate(snap_insta = case_when(
  ...
  T ~ "NA"))
# rows_without_na <- gss %>%  filter(snap_insta != "NA") %>% nrow()
...
conf_high
conf_low
#the true proportion of insta or snap users falls between 0.333 and 0.417
# gss %>% 
#   filter(snap_insta=="Yes")%>%
#   summarise(proportion_SorI= n()/rows_without_na)
...
#The 95% confidence interval for the proportion of bachelors and graduates that do not 
# use twitter is 0.724 to 0.81
conf_low_notwitter
conf_high_notwitter

#The 95% confidence interval for the proportion of bachelors and graduates that do  
# use twitter is 0.155 to 0.311
conf_low_usetwitter
...
plot_bike_week %>% ggplot() +
  ...
  geom_area(aes(x=week,y=percentage_change_expected,fill=fillcolor)) + 
  geom_rug(aes(x=week, fill= ifelse(fillcolor, "red", "green")), alpha=1) +
  facet_wrap(~year) +
