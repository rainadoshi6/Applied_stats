...
plot_bike_week %>% ggplot(aes(x=week, y=percentage_change_expected)) +
  ...
  geom_line(aes(x=week, y=percentage_change_expected)) + 
  geom_area(aes(x=week,y=percentage_change_expected)) +
  geom_ribbon(aes(ymin=0,ymax=percentage_change_expected+up, group=year),fill="#7DCD85",alpha=0.4) +
  geom_ribbon(aes(ymin=0,ymax=percentage_change_expected+down, group=year),fill="#CB454A",alpha=0.4)+
  geom_rug(aes(x=week), color=ifelse(plot_bike_week$fillcolor,"green","red"), sides="b") +
  facet_wrap(~year) +
  scale_y_continuous(labels = scales::percent) + 
  theme_bw() + 
  theme(legend.position = "none") +
