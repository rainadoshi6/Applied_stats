...
author: "Alex Kubbinga, Clara Moreno Sanchezu, Jean Huang, Raghav Mehta, Raina Doshi, Yuan Gao"
date: "`r Sys.Date()`"
output:
  html_document:
    theme: flatly
...
gss %>%
  filter(snapchat != "NA") %>%
  mutate(snapchat = factor(snapchat,levels = c("Yes","No"))) %>%
  # Specify the variable of interest
  specify(response = snapchat) %>%
...
gss <- gss %>%
  mutate(degree = factor(degree, levels=c("Lt high school","High school", "Junior college", "Bachelor", "Graduate")),
         bachelor_or_graduate = case_when(
           degree == "Bachelor" | degree == "Graduate" ~ "Yes",
           degree == "Bachelor" & degree == "Graduate" ~ "No",
           T ~ "NA")
         )
# gss %>%
#   filter(bachelor_or_graduate!="NA") %>%
#   prop_test(bachelor_or_graduate~NULL)
