...
Since there are 11 different currencies reported in the dataset, we want to unify all currencies into USD for the convenience of comparison analysis afterwards:
...
ask_a_manager_2021 <- ask_a_manager_2021 %>% 
  mutate(converted_salary = case_when(
    currency == "GBP" ~ round(annual_salary*as.numeric(first(GBPUSD))),
    currency == "AUD/NZD" & country == "Australia" ~ round(annual_salary*as.numeric(first(AUDUSD))),
    currency == "AUD/NZD" & country == "New Zealand" ~ round(annual_salary*as.numeric(first(NZDUSD))),
    currency == "CAD" ~ round(annual_salary*as.numeric(first(CADUSD))),
    currency == "EUR" ~ round(annual_salary*as.numeric(first(EURUSD))),
    TRUE ~ round(annual_salary)
  ))

# Can see that the conversion works as wanted.
ask_a_manager_2021 %>%  select(country, annual_salary, converted_salary)
```
...
ask_a_manager_2021 %>% 
  ...
  scale_fill_gradient(low="pink", high="sky blue")+
  labs(
    title="Salary Gap at different Education Level",
    y="Salary Gap in $",
    x=""
  )+
  theme_wsj()+
...
<<<<<<< HEAD
  ...
  theme_wsj()+
=======
  geom_point(aes(x = Education, y=mean_salary_by_edu, color = gender), size = 5)+
  theme_bw()+
>>>>>>> 6ae12950d8c1bde00349d71b6ea96c374744fab4
...
ask_a_manager_2021 %>% 
  ...
  group_by(race) %>% 
  filter(n() > 500) %>% 
 ggplot( aes(x=race, y=salary, color=race)) +
