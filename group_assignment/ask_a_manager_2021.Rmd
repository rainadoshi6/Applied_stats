...
```
Next, we want to creat a tibble with the currency values (exchange rate based on USD) taken from Oanda as reference in converting the top five currencies in the original dataset.
...
```{r}
ask_a_manager_2021 <- ask_a_manager_2021 %>% 
  mutate(salary_converted = case_when(
...
ask_a_manager_2021 %>%  select(country, annual_salary, salary_converted)
```
