...
currency_df <- tibble(
  value = c(first(GBPUSD),first(AUDUSD),first(CADUSD),first(EURUSD)),
  name = c("GBP", "AUD","CAD","EUR")
)
...
#creating a new column where all Top 5 most common currencies are converted to USD
ask_a_manager_2021 %>% 
  mutate(converted_salary_usd = case_when(
    currency == "GBP" ~ annual_salary_usd*currency_df$value[currency_df$name=="GBP"],
    currency == "AUD" ~ annual_salary_usd*currency_df$value[currency_df$name=="AUD"],
    currency == "CAD" ~ annual_salary_usd*currency_df$value[currency_df$name=="CAD"],
    currency == "EUR" ~ annual_salary_usd*currency_df$value[currency_df$name=="EUR"],
    T ~ annual_salary
  ))
...
>>>>>>> 9b486f73de9457cda169d9442050a5ecf68ae637
