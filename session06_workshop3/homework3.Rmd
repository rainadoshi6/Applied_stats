...
We can see that there are 1004 missing values
...
#std_error is the standard deviation of the sample mean


std_error <- sqrt((0.331* 0.669)/(8906+4404))

t_critical <- qt(0.975,4404-1)
t_critical

margin_of_error <- t_critical * std_error

phy_3plus_low <- 0.331 - margin_of_error
phy_3plus_high <- 0.331 + margin_of_error
...
yrbss_filtered <- yrbss %>%filter(!is.na(physical_3plus))

ggplot(yrbss_filtered, aes(x = weight , y = physical_3plus)) +
  geom_boxplot()+
  labs(title = "Boxplot of Weight vs Active for more than 3 days",
       x = "Weight",
       y = "Active more than 3 days")

```

## Confidence Interval

We can see that the median weight of the sample who are physically active for more than three days is greater than the median of the sample who are active for lesser than three days. This may be because of higher weight of muscle or bone due to working out/exercising. 
...
#null_dist%>% get_pvalue(obs_stat = stat , direction="both")
...
# movies <- read_csv("/Users/huangxuejing/Desktop/AM-01/am01/data/movies.csv")
...
ggplot(movies_comp,mapping = aes(x=mean_rating,y=director))+
  ...
  labs(title = "Do Spielberg and Burton have the same mean IMDB ratings?",subtitle = "95% confidence interval overlap",x = "Mean IMDB Rating")
...
H1:the mean IMDB rating for Steven Spielberg is higher than the mean IMDB rating for Tim Burton
...
movies_hypo <- movies %>% 
  group_by(director) %>% 
  filter(director %in% c("Steven Spielberg","Tim Burton")) %>% 
  mutate(Is_SS = case_when(director == "Steven Spielberg" ~ "Yes", director == "Tim Burton" ~ "No"))

t.test(rating ~ Is_SS, data = movies_hypo)
...
visualize(boot_ratings) + 
  ...
  labs(title='Bootstrap CI for Spielberg films',
       subtitle = 'Formula CI shown with dotted red lines')+
  geom_vline(xintercept = director_ci$rating_low, colour = "red", linetype="dashed", size=1.2)+
  geom_vline(xintercept = director_ci$rating_high, colour = "red", linetype="dashed", size=1.2)+
  theme_bw()+
  NULL
```
