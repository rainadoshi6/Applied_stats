...
tidy_GDP_data  <- UN_GDP_data %>% 
  ...
  mutate(volume_bn = volume / 10^9, indicator = case_when (
    IndicatorName == "Household consumption expenditure (including Non-profit institutions serving households)" ~ "Household expenditure",
    IndicatorName == "Exports of goods and services" ~ "Exports",
    IndicatorName == "Imports of goods and services" ~ "Imports",
    IndicatorName == "General government final consumption expenditure" ~ "Government expenditure",T~"NA")) %>% 
  filter(indicator!="NA")
...
tidy_GDP_country <- tidy_GDP_data %>% filter (Country %in% country_list)
...
```
```{r}
# ggplot(tidy_GDP_country,aes(x=year,y=volume_bn,color = indicator))+
#   geom_line()+
#   facet_wrap(~Country)
