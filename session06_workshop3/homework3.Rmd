...
query1 <- cdc_data %>%
  filter(death_yn %in% c("Yes","No") & !age_group %in% c("Missing","NA") & sex %in% c("Male","Female")) %>% 
  group_by(age_group,sex, death_yn) %>% 
  summarise(count=n())
...
query2 <- cdc_data %>%
  filter(icu_yn %in% c("Yes","No") & !age_group %in% c("Missing","NA") & sex %in% c("Male","Female")) %>% 
...
# calculate icu rate
query2_tibble %>% 
  pivot_wider(names_from = icu_yn,values_from=count) %>% 
  mutate(icu_rate = Yes/(No+Yes))
