...
movies_comp %>% 
  ...
  ggrepel::geom_text_repel(aes(x=mean_rating, label = round(mean_rating, 2))) +
  ggrepel::geom_text_repel(aes(x=rating_low, label = round(rating_low, 2))) +
  ggrepel::geom_text_repel(aes(x=rating_high, label = round(rating_high, 2))) +
...
```{r try to load datasets, eval=FALSE}
install.packages("RSocrata")
library(RSocrata)
cdc <- read.socrata("https://data.cdc.gov/resource/vbim-akqf.csv")
```
